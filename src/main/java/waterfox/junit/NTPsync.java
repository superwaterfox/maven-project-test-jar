...
public class NTPsync {
	...
	private static Date getNetDate(String hostName) {
		...
		try {
			...
			Logger.getLogger(NTPsync.class.getName()).log(Level.SEVERE, null, ex);
	...
	public static boolean offLine() {
		...
		try {
			...
			Scanner scanner = new Scanner(s, "GBK");
			int i = 0;
			while (scanner.hasNextLine() && i < 3) {// 只读取前三行的返回结果
				i++;
				String nextLine = scanner.nextLine();
				// 内容中不为空，并且是有成功的
				if (!nextLine.startsWith("") && nextLine.startsWith("Reply from") || nextLine.startsWith("来自")) {
					// break;
					return false;
				}
			}
	...
	/**
	 * 拼凑时间格式，然后通过调用本地命令date和time修改计算机时间
	 * 
	 * @param date
	 */
